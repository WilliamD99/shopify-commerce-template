...
export default function AnimateOnScroll({ children, selector, direction, stagger }) {
    const ele = useRef()
        ...
        if (selector) {
            ...
            ScrollTrigger.batch(selector, {
                ...
                onEnter: (batch) => {
                    ...
                    gsap.to(batch, { autoAlpha: 1, ...startLocation, ease: "Sine.easeInOut", stagger: animStagger })
                }
            })
        } else {
    ...
    return (
        <>
            <div className='invisible' ref={ele}>{children}</div>
        </>
    )
}
