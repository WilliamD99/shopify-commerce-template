...
export default function FilterMenu({isLoading}) {
    ...
    useEffect(() => {
        if (collectionQuery.data) setCollections(collectionQuery.data.data.collections.edges)
    }, [collectionQuery.isLoading])

    return (
        <>
            <div id="filter" className="filter-menu overflow-scroll overflow-x-hidden sb-custom w-3/12 xl:w-2/12 bg-slate-50 px-3 py-5">
                    ...
                    <div className='relative sb-custom h-full flex flex-col space-y-2 overflow-scroll overflow-x-hidden py-4 pl-2'>
                        {
                            ...
                            collections.map(e => (
                                <div key={e.node.handle} className="flex flex-row justify-between items-center">
                                    <Link className={`${decodeURIComponent(routerQuery.col) === e.node.id ? 'font-semibold' : ''}`} href={{pathname: "/shop/products-in-collection/", query: { col: encodeURIComponent(e.node.id) }}}>
                                        {e.node.title}
                                    </Link>
                ...
                <div id="price-filter" className='py-5 mb-10 h-56 flex flex-col space-y-3'>
                    <PriceFilter />
                </div>
            </div>
        </>
