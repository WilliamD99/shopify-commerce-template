...
export default function SearchDrawer({ open, setOpen }) {
          ...
          <div className="grid grid-cols-1 gap-y-4 py-5 sb-custom">
            {data.data.products.edges.map((e, i) => (
              <Link
                ...
                className="grid grid-cols-2 gap-x-5 hover:bg-slate-100 py-2 px-2"
                key={`search-${i}`}
              >
  ...
  return (
    <>
      ...
      >
        <Box className="w-80 pt-28 px-5 flex flex-col space-y-3 relative h-full bg-white">
          <form id="search-bar" className="flex flex-col space-y-2 relative">
            <div className="flex flex-row justify-between items-center">
              ...
              <AiOutlineClose className="cursor-pointer" onClick={setOpen} />
            </div>
            <TextField
              className="w-full"
              id="search"
